...
function EntryList({ entries, savedEntries, accentColor, labelKey, valueKey, unitLabel, onRemove, onDeleteSaved }: {
    ...
    const borderSaved = `border-${accentColor}-300`;
    const borderPending = `border-${accentColor}-200`;
    const textAccent = `text-${accentColor}-700`;
    const textAccentSm = `text-${accentColor}-600`;

    return (
        <div className="flex flex-col gap-2">
            {savedEntries.map((e) => (
                <div key={`saved-${e.id}`} className={`relative flex justify-between items-center px-3 py-2 bg-white border ${borderSaved} rounded-lg pr-10`}>
                    <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-[10px] text-[#555555] truncate">{(e as any)[labelKey]}{e.jam ? ` · ${e.jam}` : ''}</span>
                    </div>
                    {e.id && onDeleteSaved && (
                        <button type="button" onClick={() => onDeleteSaved(e.id!)}
                            className="absolute right-1.5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-lg bg-red-50 border border-red-200 text-red-600 hover:bg-red-100 flex items-center justify-center transition-colors">
            ...
            {entries.map((e, idx) => (
                <div key={`pending-${idx}`} className={`relative flex justify-between items-center px-3 py-2 bg-[#FAFAFA] border border-dashed ${borderPending} rounded-lg pr-10`}>
                    <div className="flex flex-col min-w-0">
                        <span className={`text-xs font-mono font-bold ${textAccent}`}>{(e.jumlah || 0).toLocaleString('id-ID')} <span className={`text-[10px] ${textAccentSm}`}>{unitLabel}</span></span>
                        <span className="text-[10px] text-[#555555] truncate">{(e as any)[labelKey]}{e.jam ? ` · ${e.jam}` : ''} <span className={`text-[9px] ${textAccentSm}`}>(baru)</span></span>
                    </div>
...
function timeInput(value: string, prev: string, onChange: (v: string) => void) {
    return (
        <div className="space-y-1.5 w-full">
            <label className="font-medium text-[#141414] uppercase tracking-wider block text-left text-[10px]">Jam</label>
            <input
                ...
                className="w-full bg-white border border-[#DCDCDC] rounded-lg py-2.5 px-3 text-[#141414] placeholder-[#B5B5B5] focus:ring-1 focus:ring-amber-500 focus:border-amber-500 text-sm font-mono transition-all"
            />
        </div>
    );
}
    ...
    return (
        <>
            <div className="w-full xl:flex-1 xl:overflow-y-auto pr-1 sm:pr-2 scrollbar-hide">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <Card title="Loading Batubara" icon="local_shipping" color="orange">
                        ...
                        <div className="space-y-1.5 w-full">
                            <label className="font-medium text-[#141414] uppercase tracking-wider block text-left text-[10px]">Hopper Aktif</label>
                            <select className={`w-full bg-white border border-[#DCDCDC] rounded-lg py-2.5 px-3 focus:ring-1 focus:ring-orange-500 focus:border-orange-500 text-sm font-bold transition-all ${!espValues.hopper ? 'text-[#9A9A9A]' : 'text-[#141414]'}`}
                                value={(espValues.hopper as string) || ''} onChange={e => onEspChange?.('hopper', e.target.value || null)}>
                                <option value="" className="text-[#9A9A9A] bg-white">Pilih...</option>
                                <option value="A" className="text-[#141414] bg-white font-bold">Hopper A</option>
                                <option value="B" className="text-[#141414] bg-white font-bold">Hopper B</option>
                                <option value="AB" className="text-[#141414] bg-white font-bold">Hopper AB</option>
                            </select>
                        </div>
                        <div className="space-y-1.5 w-full">
                            <label className="font-medium text-[#141414] uppercase tracking-wider block text-left text-[10px]">Conveyor Status</label>
                            <select className={`w-full bg-white border border-[#DCDCDC] rounded-lg py-2.5 px-3 focus:ring-1 focus:ring-orange-500 focus:border-orange-500 text-sm font-bold transition-all ${!espValues.conveyor ? 'text-[#9A9A9A]' : 'text-[#141414]'}`}
                                value={(espValues.conveyor as string) || ''} onChange={e => onEspChange?.('conveyor', e.target.value || null)}>
                                <option value="" className="text-[#9A9A9A] bg-white">Pilih...</option>
                                <option value="AB" className="text-[#141414] bg-white font-bold">Conveyor AB</option>
                                <option value="A" className="text-[#141414] bg-white font-bold">Conveyor A</option>
                                <option value="B" className="text-[#141414] bg-white font-bold">Conveyor B</option>
                            </select>
                        </div>
                    </Card>
                        ...
                        {allInSolar.length > 0 && (
                            <div className="mb-1">
                                ...
                                <div className="flex justify-between items-center px-1 mt-2 mb-1">
                                    <span className="text-[10px] text-[#141414] uppercase tracking-wider font-semibold">Total</span>
                                    <span className="text-sm font-mono font-bold text-amber-700">{totalInSolar.toLocaleString('id-ID')} L</span>
                                </div>
                                <div className="h-px bg-[#E8E8E8] mb-3" />
                            </div>
                        )}
                        <button type="button" onClick={() => setShowSolarModal(true)}
                            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-[#422006] text-sm font-bold transition-colors">
                    ...
                    <Card title="Permintaan Solar" icon="upload" color="rose">
                        {allOutSolar.length > 0 && (
                            <div className="mb-1">
                                ...
                                <div className="flex justify-between items-center px-1 mt-2 mb-1">
                                    <span className="text-[10px] text-[#141414] uppercase tracking-wider font-semibold">Total</span>
                                    <span className="text-sm font-mono font-bold text-rose-700">{totalOutSolar.toLocaleString('id-ID')} L</span>
                                </div>
                                <div className="h-px bg-[#E8E8E8] mb-3" />
                            </div>
                        )}
                        <button type="button" onClick={() => setShowOutModal(true)}
                            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-400 text-white text-sm font-bold transition-colors">
                            <span className="material-symbols-outlined text-[18px]">add_circle</span>
                            Tambah Permintaan
                        </button>
                    </Card>
            ...
            <Modal open={showSolarModal} onClose={() => { setShowSolarModal(false); setSolarForm(EMPTY_SOLAR); }} title="Kedatangan Solar" color="amber">
                ...
                <div className="space-y-1.5 w-full">
                    <label className="font-medium text-[#141414] uppercase tracking-wider block text-left text-[10px]">Perusahaan</label>
                    <input type="text" value={solarForm.perusahaan} onChange={e => setSolarForm({ ...solarForm, perusahaan: e.target.value })}
                        placeholder="Nama perusahaan..."
                        className="w-full bg-white border border-[#DCDCDC] rounded-lg py-2.5 px-3 text-[#141414] placeholder-[#B5B5B5] focus:ring-1 focus:ring-amber-500 focus:border-amber-500 text-sm transition-all" />
                </div>
                <button type="button" onClick={saveSolar} disabled={!solarForm.jam || !solarForm.jumlah || !solarForm.perusahaan}
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 disabled:opacity-40 disabled:cursor-not-allowed text-[#422006] font-bold text-sm transition-colors mt-1">
            ...
            <Modal open={showOutModal} onClose={() => { setShowOutModal(false); setOutForm(EMPTY_OUT); setTujuanMode('Bengkel'); }} title="Permintaan Solar" color="rose">
                ...
                <div className="space-y-1.5 w-full">
                    <label className="font-medium text-[#141414] uppercase tracking-wider block text-left text-[10px]">Tujuan Permintaan</label>
                    ...
                    }} className="w-full bg-white border border-[#DCDCDC] rounded-lg py-2.5 px-3 text-[#141414] focus:ring-1 focus:ring-rose-500 focus:border-rose-500 text-sm transition-all">
                    ...
                    {tujuanMode === 'Lainnya' && (
                        <input type="text" value={outForm.tujuan} onChange={e => setOutForm({ ...outForm, tujuan: e.target.value })}
                            placeholder="Tulis tujuan..." className="mt-2 w-full bg-white border border-[#DCDCDC] rounded-lg py-2.5 px-3 text-[#141414] placeholder-[#B5B5B5] focus:ring-1 focus:ring-rose-500 focus:border-rose-500 text-sm transition-all" />
                    )}
                </div>
