        ...
        <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-[1400px] mx-auto space-y-6">
            <header className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-black tracking-tight text-slate-900">User & WhatsApp</h2>
                        <p className="text-slate-500 text-sm mt-1">Kelola operator + nomor WhatsApp untuk notifikasi</p>
                    </div>
                </div>
            </header>

            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-5 py-3 border-b border-slate-200 bg-gradient-to-r from-surface-highlight/50 to-transparent flex items-center justify-between">
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50">
                                <th className="text-left text-xs text-slate-500 uppercase tracking-wider py-3 px-5">Nama</th>
                                <th className="text-left text-xs text-slate-500 uppercase tracking-wider py-3 px-3">Role</th>
                                <th className="text-left text-xs text-slate-500 uppercase tracking-wider py-3 px-3">Grup</th>
                                <th className="text-left text-xs text-slate-500 uppercase tracking-wider py-3 px-3">WhatsApp</th>
                                <th className="text-center text-xs text-slate-500 uppercase tracking-wider py-3 px-5">Aksi</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {loading && (
                                <tr><td colSpan={5} className="text-center py-8 text-slate-500 text-sm">Memuat...</td></tr>
                            )}
                            {!loading && rows.length === 0 && (
                                <tr><td colSpan={5} className="text-center py-8 text-slate-500 text-sm">Tidak ada operator. Pastikan tabel operators terisi.</td></tr>
                            )}
                            {rows.map((op) => (
                                <tr key={op.id} className="hover:bg-slate-50 transition-colors">
                                    <td className="py-3 px-5">
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 border border-slate-300">
                                                {op.name.charAt(0)}
                                            </div>
                                            <div>
