...
function actionStyle(action: string): string {
    switch (action) {
        case 'inserted': return 'bg-emerald-50 text-emerald-700 border-emerald-200';
        case 'updated':  return 'bg-blue-50 text-blue-700 border-blue-200';
        case 'sheet_not_found': return 'bg-amber-50 text-amber-700 border-amber-200';
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-[1400px] mx-auto space-y-6">
            <header className="flex items-center gap-4">
                <div className="p-3 bg-cyan-50 border border-cyan-100 rounded-xl">
                    <span className="material-symbols-outlined text-cyan-500 text-2xl">sync</span>
                </div>
                <div>
                    <h2 className="text-2xl font-black tracking-tight text-slate-900">Sync dari Google Sheets</h2>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                    <button
                        ...
                        disabled={loading}
                        className="px-5 py-2.5 bg-white border border-slate-300 hover:bg-slate-100 disabled:opacity-50 text-slate-800 rounded-xl font-semibold transition-colors flex items-center gap-2"
                    >
            ...
            {result && (
                <div className="space-y-6">
                                ...
                                <tbody>
                                    {(result.shiftResults ?? []).map((r, i) => (
                                        <tr key={i} className="border-t border-slate-100">
                                            <td className="px-3 py-2 text-slate-800">{r.date}</td>
                                            <td className="px-3 py-2 capitalize text-slate-800">{r.shift}</td>
                                            <td className="px-3 py-2">
                                                <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold border ${actionStyle(r.action)}`}>
                                                    {r.action}
                                                </span>
                                            </td>
                                            <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.raw_totalizer_steam_a)}</td>
                                            <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.raw_totalizer_steam_b)}</td>
                                            <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.batubara_a)}</td>
                                            <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.batubara_b)}</td>
                    ...
                    <section>
                        <h3 className="text-lg font-bold text-slate-900 mb-3 flex items-center gap-2">
                            <span className="material-symbols-outlined text-emerald-600">today</span>
                                    ...
                                    {(result.dailyResults ?? []).map((r, i) => {
                                        ...
                                        return (
                                            <tr key={i} className="border-t border-slate-100">
                                                ...
                                                <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.fields.prod_boiler_a_24)}</td>
                                                <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.fields.prod_boiler_b_24)}</td>
                                                <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.fields.inlet_turbine_24)}</td>
                                                <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.fields.power_ubb_totalizer)}</td>
                                                <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.fields.power_pabrik2_totalizer)}</td>
                                                <td className="px-3 py-2 text-right text-slate-600 font-mono">{fmt(r.fields.power_stg_ubb_totalizer)}</td>
                                                <td className="px-3 py-2 text-right text-slate-600 font-mono">{coalSum ? fmt(coalSum) : '—'}</td>
                                                <td className="px-3 py-2 text-xs text-slate-400">{r.note ?? ''}</td>
                                            </tr>
                                        );
                                    })}
