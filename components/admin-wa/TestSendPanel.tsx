...
export default function TestSendPanel() {
    ...
    return (
        <div className="space-y-4">
            <div>
                <h3 className="text-sm font-bold text-slate-900">Test Send</h3>
                <p className="text-xs text-slate-500 mt-1">Kirim pesan tes ke group terdaftar, atau ke nomor pribadi/JID manual untuk verifikasi koneksi Fonnte.</p>
            </div>

            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4 max-w-2xl">
                <div>
                    <label className="block text-xs text-slate-500 uppercase mb-1.5">Akun Fonnte (pengirim)</label>
                    ...
                    <p className="text-xs text-slate-500 mt-1">Menentukan nomor WA pengirim untuk semua tombol kirim di panel ini.</p>
                </div>

                <div className="flex gap-1 p-1 bg-slate-100 rounded-lg w-fit">
                    <button onClick={() => setMode('group')}
                        className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all cursor-pointer ${mode === 'group' ? 'bg-primary text-white' : 'text-slate-500 hover:text-slate-900'}`}>
                        Group terdaftar
                    </button>
                    <button onClick={() => setMode('manual')}
                        className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all cursor-pointer ${mode === 'manual' ? 'bg-primary text-white' : 'text-slate-500 hover:text-slate-900'}`}>
                        Nomor / JID manual
                    </button>
                </div>
                ...
                {mode === 'group' ? (
                    <div>
                        <label className="block text-xs text-slate-500 uppercase mb-1.5">Target Group</label>
                        <select value={target} onChange={e => setTarget(e.target.value)}
                            className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-slate-800 text-sm focus:outline-none focus:border-primary">
                            <option value="">— pilih target —</option>
                            {groups.map(g => <option key={g.id} value={g.fonnte_target}>{g.label} ({g.key})</option>)}
                        </select>
                        {groups.length === 0 && <p className="text-xs text-slate-500 mt-1">Belum ada group terdaftar. Tambah dulu di tab Groups WA.</p>}
                    </div>
                ) : (
                    <div>
                        <label className="block text-xs text-slate-500 uppercase mb-1.5">Nomor pribadi / Group JID</label>
                        ...
                        <p className="text-xs text-slate-500 mt-1">Format Fonnte: <code>628xxx</code> tanpa &quot;+&quot;, atau group JID berakhiran <code>@g.us</code>.</p>
                    </div>
                )}

                <div>
                    <label className="block text-xs text-slate-500 uppercase mb-1.5">Pesan Custom</label>
                ...
                <div className="flex gap-2 justify-end">
                    <button onClick={sendDefault} disabled={busy || !effectiveTarget}
                        className="px-4 py-2 text-sm bg-white border border-slate-300 hover:bg-slate-100 disabled:opacity-40 text-slate-800 rounded-lg cursor-pointer">
                        Kirim Pesan Default
                    </button>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4 max-w-2xl">
                <div className="flex items-center justify-between">
                    <div>
                        <h4 className="text-sm font-bold text-slate-900">Preview Template</h4>
                        <p className="text-xs text-slate-500 mt-1">Lihat hasil render template dengan variable ({'{{links}}'}, {'{{shift}}'}, dll) terisi. Opsional kirim ke target di atas.</p>
                    </div>
                    <button onClick={refreshPreview} disabled={previewLoading}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs bg-white border border-slate-300 hover:bg-slate-100 disabled:opacity-40 text-slate-800 rounded-lg cursor-pointer">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                    <div className="sm:col-span-2">
                        <label className="block text-xs text-slate-500 uppercase mb-1.5">Template</label>
                    ...
                    <div>
                        <label className="block text-xs text-slate-500 uppercase mb-1.5">Shift</label>
                    ...
                    <div>
                        <label className="block text-xs text-slate-500 uppercase mb-1.5">Group</label>
                    ...
                    <div className="sm:col-span-4">
                        <label className="block text-xs text-slate-500 uppercase mb-1.5">Tanggal</label>
                ...
                <div>
                    <label className="block text-xs text-slate-500 uppercase mb-1.5">Hasil Render</label>
                    <pre className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-3 text-slate-800 text-xs font-mono whitespace-pre-wrap break-words max-h-96 overflow-auto">
