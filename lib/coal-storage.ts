...
// ── Cadangan: stok cukup berapa hari ────────────────────────────────────────

/**
 * Batas warna "stok cukup berapa hari": di bawah `kritis` merah, di bawah `waspada`
 * kuning, selebihnya hijau. Ubah di sini kalau kebijakan cadangan pembangkit berubah.
 */
export const BATAS_CADANGAN_HARI = { kritis: 7, waspada: 14 };

export type StatusCadangan = 'kritis' | 'waspada' | 'aman';

/** Pemakaian boiler rata-rata; lihat fetchDenah untuk cara merata-ratakannya. */
export interface PemakaianHarian {
    tonPerHari: number;
    /** Berapa laporan harian yang dirata-ratakan (1–7). */
    jumlahHari: number;
}

/**
 * Rata-rata pemakaian dari hingga `maksHari` laporan harian TERBARU yang benar-benar
 * membakar batubara (ton > 0). Hari berhenti total dilewati: yang ditanyakan "cukup
 * berapa hari" adalah laju saat pembangkit jalan, dan hari nol akan membuat stok
 * tampak lebih awet dari kenyataannya. Tanpa satu pun hari berisi → null.
 *
 * `ton` per baris = Boiler A + Boiler B. Sengaja tidak memakai grand_total_24: kolom
 * itu pernah tersimpan hanya berisi satu boiler (5 dari 60 hari, per Sep 2026).
 */
export function rataRataPemakaian(
    baris: { tanggal: string; ton: number }[],
    maksHari = 7,
): PemakaianHarian | null {
    const dipakai = baris
        .filter(b => Number.isFinite(b.ton) && b.ton > 0)
        .sort((a, b) => b.tanggal.localeCompare(a.tanggal))
        .slice(0, maksHari);
    if (dipakai.length === 0) return null;
    const total = dipakai.reduce((t, b) => t + b.ton, 0);
    return { tonPerHari: total / dipakai.length, jumlahHari: dipakai.length };
}

/**
 * Stok ini cukup untuk berapa hari pada laju pemakaian sekarang.
 * null kalau lajunya tidak diketahui — pembagian dengan nol tidak boleh jadi "∞ hari".
 */
export function hariCadangan(stokTon: number, tonPerHari: number): number | null {
    if (!Number.isFinite(tonPerHari) || tonPerHari <= 0) return null;
    return Math.max(0, stokTon) / tonPerHari;
}

export function statusCadangan(hari: number): StatusCadangan {
    if (hari < BATAS_CADANGAN_HARI.kritis) return 'kritis';
    if (hari < BATAS_CADANGAN_HARI.waspada) return 'waspada';
    return 'aman';
}

/**
 * '< 1' · '1,6' · '38'. Di bawah 10 hari satu desimal masih berarti (setengah hari
 * itu 1–2 shift); di atas itu desimal cuma kesan presisi palsu dari angka estimasi.
 */
export function formatHari(hari: number): string {
    if (hari < 1) return '< 1';
    if (hari < 10) return hari.toLocaleString('id-ID', { maximumFractionDigits: 1 });
    return Math.round(hari).toLocaleString('id-ID');
}
