...
// Warna per grup dijaga sama semantiknya dengan GROUP_DOT di app/home/page.tsx
// (A cyan, B blue, C violet, D emerald, ND amber) supaya grup dikenali sama di
// kedua halaman.
const GROUP_CONFIG = [
  { group: 'A', label: 'Group A', color: 'from-cyan-50 to-white', border: 'border-cyan-200', accent: 'text-cyan-700', dot: 'bg-cyan-500', ring: 'ring-cyan-200' },
  { group: 'B', label: 'Group B', color: 'from-blue-50 to-white', border: 'border-blue-200', accent: 'text-blue-700', dot: 'bg-blue-500', ring: 'ring-blue-200' },
  { group: 'C', label: 'Group C', color: 'from-violet-50 to-white', border: 'border-violet-200', accent: 'text-violet-700', dot: 'bg-violet-500', ring: 'ring-violet-200' },
  { group: 'D', label: 'Group D', color: 'from-emerald-50 to-white', border: 'border-emerald-200', accent: 'text-emerald-700', dot: 'bg-emerald-500', ring: 'ring-emerald-200' },
  { group: 'ND', label: 'Normal Day', color: 'from-amber-50 to-white', border: 'border-amber-200', accent: 'text-amber-700', dot: 'bg-amber-500', ring: 'ring-amber-200' },
];
...
function CompanyBadge({ company }: { company?: string }) {
  ...
  return (
    <span className="px-1.5 py-0.5 text-[10px] font-medium rounded bg-slate-100 text-slate-500 uppercase tracking-wide">
      {short}
    </span>
  );
}
...
export default function LoginPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-50">
        <div className="animate-pulse text-neutral-400 text-sm font-medium">Memuat…</div>
      </div>
    );
  }
  ...
  return (
    <div className="light-canvas min-h-screen flex items-center justify-center px-4 bg-neutral-50 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm">
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <h1 className="text-xl font-bold text-slate-900">Web Utilitas Batubara</h1>
            <p className="text-xs text-neutral-400 mt-1">Operator Panel Login</p>
          </div>

          {/* Step 1: Group Selection */}
          {!selectedGroup && (
            <div className="space-y-3">
              <p className="text-sm font-medium text-slate-500 text-center mb-4">Pilih Grup Shift</p>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400">{count} personil</p>
                        </div>
                      </div>
                      <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 group-hover:text-slate-500 transition-colors text-[20px]">
                        chevron_right
                      </span>
                    </button>
                  );
          ...
          {selectedGroup && !selectedOp && (
            <div className="space-y-3">
              <div className="flex items-center gap-2 mb-3">
                <button onClick={handleBack} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors cursor-pointer">
              ...
              <div className="max-h-[360px] overflow-y-auto space-y-1">
                ...
                {organik.length > 0 && (
                  <>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2 pt-1">Organik</p>
                    {organik.map(op => (
                      <button
                        key={op.id}
                      ...
                      >
                        <div className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0">
                          <span className="text-xs font-bold text-slate-600">{op.name.charAt(0)}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-slate-800 truncate">{op.name}</p>
                          {op.jabatan && (
                            <p className="text-[11px] text-slate-500">{op.jabatan}</p>
                          )}
                        </div>
                        <span className="material-symbols-outlined text-slate-300 group-hover:text-slate-500 text-[18px]">
                          chevron_right
                        </span>
                      </button>
                    ))}
                ...
                {tad.length > 0 && (
                  <>
                    <div className="flex items-center gap-2 px-2 pt-3 pb-1">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tenaga Alih Daya</p>
                      <div className="h-px bg-slate-200 flex-1" />
                    </div>
                    {tad.map(op => (
                      <button
          ...
          {selectedOp && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-2">
                <button onClick={() => setSelectedOp(null)} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors cursor-pointer">
                  <span className="material-symbols-outlined text-slate-500 text-[20px]">arrow_back</span>
                </button>
                <p className="text-sm font-medium text-slate-500">Konfirmasi Login</p>
              </div>
              ...
              <div className={`p-4 rounded-xl border ${activeConfig?.border} bg-gradient-to-br ${activeConfig?.color}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-base font-semibold text-slate-900 truncate">{selectedOp.name}</p>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      {selectedOp.jabatan && (
                        <span className="text-xs text-slate-600">{selectedOp.jabatan}</span>
                      )}
                      {selectedOp.group && (
                    ...
                    {selectedOp.nik && (
                      <p className="text-[11px] text-slate-400 mt-1">NIK: {selectedOp.nik}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl text-base font-semibold transition-all duration-300 cursor-pointer
                  bg-primary text-white shadow-md shadow-primary/20 hover:shadow-primary/30 hover:bg-primary/90 active:scale-[0.99]"
              >
                MASUK
              </button>
            </div>
        ...
        <p className="text-center text-xs text-slate-400 mt-6">
          &copy; 2025 Web Utilitas Batubara
        </p>
      </div>
    </div>
  );
